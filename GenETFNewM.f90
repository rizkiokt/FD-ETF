...
CONTAINS
    ...
    IMPLICIT NONE
        ...
        WRITE(output_unit,*) " "
        WRITE(output_unit,*) "x = ori_xstrf/xstrf"
        WRITE(output_unit,"(2F10.4)") ori_xstrf/xstrf
        WRITE(output_unit,*) " "
    ...
    IMPLICIT NONE
        ...
        INTEGER(sik) :: ixy, it,it_bt, count_conv
        ...
        DO WHILE(absF>tol)
            ...
            ! Add more convergence criteria
            IF (ABS(1-absF/prev_absF)<tol) THEN
    ...
    IMPLICIT NONE
        ...
        REAL(sdk)    :: hom_cur, hom_leak, hn, hc, C_ks, K_ks
            ...
            DO is=1,ns-2
                ...
                IF (ineigh .NE. 0)THEN
                    ...
                    ELSE
                        ...
                        hn = hy(ineigh)
                    END IF
                    C_ks = two_third * (serpflux(ig,ixy,iz) - serpflux(ig,ineigh,iz)) * area(ixy,is,iz)
                    C_ks = C_ks / (xstrf(ig,ineigh,iz)*hn)
                    K_ks = (xstrf(ig,ixy,iz)*hc) / (xstrf(ig,ineigh,iz)*hn)
                    hom_cur = C_ks * (x(ixy)*x(ineigh)) / (x(ixy)+K_ks*x(ineigh))
                    hom_leak = hom_leak + hom_cur
                END IF
            END DO
        ...
        DO ixy=1,nxy
            ...
            DO is=1,ns-2
                ...
                IF (ineigh .NE. 0)THEN
                    ...
                    sumJkk = sumJkk + C_ks * (K_ks*x(ineigh)**2) / (x(ixy) + K_ks*x(ineigh))**2
                    gradF(ixy,ineigh) = C_ks * (x(ixy)**2) / (x(ixy) + K_ks*x(ineigh))**2

                END IF
            END DO
            gradF(ixy,ixy) = sumJkk
        END DO      
        ...
        DO is=1,ns-2
            ...
            IF (in1 .NE. 0)THEN
                ...
                ELSE
                    ...
                    hn = hy(in1)
                END IF
                C_ks = two_third * (serpflux(ig,k,iz) - serpflux(ig,in1,iz)) * area(k,is,iz)
                C_ks = C_ks / (xstrf(ig,in1,iz)*hn)
                K_ks = (xstrf(ig,k,iz)*hc) / (xstrf(ig,in1,iz)*hn)
                
                den = (x(k) + K_ks * x(in1))**3
                grad2fk(k,k) = grad2fk(k,k) + C_ks * (-2*K_ks*x(in1)**2) / den
                grad2fk(k,in1) = C_ks * (-2*K_ks*x(k)*x(in1)) / den
                grad2fk(in1,k) = grad2fk(k,in1)
                grad2fk(in1,in1) = C_ks * (-2*K_ks*x(k)**2) / den
